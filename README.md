...
Unknown777
## プライバシー
本アプリは完全オフラインで動作し、外部へのデータ送信は一切行いません。詳細は [PRIVACY.md](./PRIVACY.md) をご覧ください。
本アプリはユーザーが選択したフォルダのみにアクセスし、バックグラウンドで全ドライブを無断で走査することはありません。

注意： 本ツールはご自身が所有・管理するPC、または所有者から明確な許可を得た範囲でのみご使用ください。他人の端末を無断で検索・閲覧する行為はプライバシー侵害や不正アクセスに該当する可能性があります。作者は不適切な利用によって生じたトラブルについて責任を負いません。
