...
[最新版をダウンロード](https://github.com/Unknown777hello/FileSearchExplorer/releases/tag/v0.9.2)
...
- ファイル名・内容を全文検索
## 今回でのアップデートでの変更内容
- 「現在の走査数」が「総数」を超えてしまうことがある不具合を修正しました
- 進捗の残り4%ぐらいのところで止まる不具合を修正しました
- 進捗率（％）の計算方法を見直し、より実際の状況に近い値になるようにしました。
- 「残り◯％」の表示が、実際の進み具合とずれることがある不具合を修正しました。
- 検索で見つかった件数（結果件数）と、フォルダ内を調べた件数（走査件数）が
紛らわしく表示されることがあったのを、はっきり分けて表示するようにしました。
- 検索が完了したときに、進捗が確実に100%まで到達するようにしました。
- ステータスバーの表示が、実際の検索の進み具合とより一致するようにしました。
## 検索画面のスクリーンショット
